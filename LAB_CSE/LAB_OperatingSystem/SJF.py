...
def FCFS(processes, n, at ,bt):
	...
	for i in range(n):
		...
		process.start()
		process.join()
		print(" " + str(processes[i])+ "\t    " + str(at[i]) + "\t\t "  + str(bt[i]) + "\t\t" + str(ct[i]) + "\t\t" + str(tat[i]) + "\t\t" + str(wt[i])+ "\t\t" + str(rt[i]))
...
'''
	...
	for i in range(1, noOfProcess+1):
		process_no = i
        #burst_time = random.choice(range(1,6)) #random burst time
		burstTime = burst_time[i-1]
		#process = Process(target=Task, args=(process_no,burst_time,)) #random burst time
		process = Process(target=Task, args=(process_no,burstTime,))
